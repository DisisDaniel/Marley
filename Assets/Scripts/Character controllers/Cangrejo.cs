...
public class PlayerController : MonoBehaviour
{
    ...
    void FixedUpdate(){
        // Obtener la velocidad actual del objeto solo en el eje X.
        float velocidadActualX = gameObject.GetComponent<Rigidbody2D>().velocity.x;

        // Verificar si la velocidad actual en el eje X excede la velocidad máxima.
        if (Mathf.Abs(velocidadActualX) > 5f)
        {
            // Limitar la velocidad en el eje X al valor máximo permitido.
            gameObject.GetComponent<Rigidbody2D>().velocity = new Vector2(Mathf.Sign(velocidadActualX) * 5f, gameObject.GetComponent<Rigidbody2D>().velocity.y);
        }
    }
